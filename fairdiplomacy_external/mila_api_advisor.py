...
class milaWrapper:
    def __init__(self):
        ...
        self.last_successful_message_time = (
            None  # timestep for last message successfully sent in the current phase
        )
        self.last_comm_intent = {
            "RUSSIA": None,
            "TURKEY": None,
            "ITALY": None,
            "ENGLAND": None,
        ...
        self.prev_received_msg_time_sent = {
            "RUSSIA": None,
            "TURKEY": None,
            "ITALY": None,
            "ENGLAND": None,
        ...
        agent_config = heyhi.load_config(
            "/diplomacy_cicero/conf/common/agents/diplodocus_high.prototxt"
        )
    ...
    async def play_mila(self, args) -> None:
        ...
        self.dipcc_game = self.start_dipcc_game(power)
        self.player = Player(self.agent, power)
        ...
        while not self.game.is_game_done and not self.game.powers[power].is_eliminated():
            ...
            if current_phase is None or current_phase != phase_name:
                ...
                if prev_power_stance is not None:
                    ally_centers = []

                    ally_powers = [k for k, v in prev_power_stance.items() if v > 0]
                    ...
                    dipcc_json["phases"][0]["state"]["units"] = current_units
                    dipcc_json["phases"][0]["state"]["centers"] = current_centers
                ...
                try:
                    await self.chiron_agent.suggest_orders(orders=first_order)
    ...
    async def suggest_move(self, power_name):
        ...
        logger.info(
            f"Human has set orders: {cond_orders}; check if all orderable locations ({orderable_locs}) are set"
        )
        if len(cond_orders) != 0 and len(cond_orders) != len(orderable_locs):
            ...
            policy = self.player.get_plausible_orders_policy(self.dipcc_game)[
                power_name
            ]
            logger.info(
                f"Searching for conditional orders using {cond_orders} in {power_name}'s policy: {policy}"
            )
            hit = False
            ...
            for action, prob in policy.items():
                action = list(action)
                if (
                    all(c_order in action for c_order in cond_orders)
                    and prob > max_prob
                ):
                    hit = True
            ...
            if hit and best_cond_action != self.prev_suggest_cond_moves:
                new_order = [
                    action for action in best_cond_action if action not in cond_orders
                ]
                logger.info(f"Sending move advice at {round(time.time() * 1_000_000)}")
                await self.chiron_agent.suggest_orders(
                    new_order, partial_orders=cond_orders
                )
    ...
    async def get_should_stop(self) -> bool:
        ...
        assert close_to_deadline >= 0 or deadline == 0, (
            f"Press period is less than zero or there is no deadline: {close_to_deadline}"
        )
...
def main() -> None:
    ...
    while True:
        try:
            asyncio.run(mila.play_mila(args))
        except Exception as e:
