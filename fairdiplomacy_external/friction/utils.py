...
def get_proposal_move_dict(dipcc_game, msg):
    ...
    for move in extracted_moves:
        ...
        possible = is_power_unit(power_units, unit_loc_key)
...
def load_offline_game_and_test_utils(input_file, target_phase='S1901M', target_message='None', our_power='AUSTRIA',target_power='AUSTRIA'):
    ...
    for phase in data['phases']:
        ...
        for msg in phase['messages']:
            ...
            msg_tuple = msg_to_move_dict(msg_tuple)
            prev_extracted_moves[pair_power_str] = copy.deepcopy(msg_tuple['extracted_moves'])
            prev_messages[pair_power_str] = copy.deepcopy(msg_tuple)
    ...
    if found_msg:   
        sender = found_msg['sender'],
        recipient = found_msg['recipient']
        pair_power_str = '-'.join(sorted([sender, recipient]))
        msg_tuple = {'sender': sender, 'recipient': recipient, 'message': found_msg['message']}
        msg_tuple = msg_to_move_dict(msg_tuple)
        is_friction_in_proposal(dipcc_game, cicero_player, found_msg, our_power)
